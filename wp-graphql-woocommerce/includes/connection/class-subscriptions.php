 ...
 *
 * Registers connections to Subscription
 *
...
class Subscriptions {
            ...
            self::get_connection_config(
                array(
                    ...
                    'resolve'        => function( $source, array $args, AppContext $context, ResolveInfo $info ) {
                        ...
                        $subscription = \wcs_get_subscription( $source->ID );
                        $order_ids = array_values($subscription->get_related_orders());
                        //error_log('--------------Order id --------'. $source->ID .'----------'. print_r($order_ids, true));

                        $resolver->set_query_arg( 'post__in', $order_ids );
	...
	private static function get_customer_order_connection( $resolver, $customer ) {
        ...
        //$meta_value = ! empty( $customer->get_billing_email() ) ? $customer->get_billing_email() : $customer->get_id();
			...
			array(
				...
				'resolve'        => function( $source, array $args, AppContext $context, ResolveInfo $info ) use ( $post_object ) {
					...
					$args = $not_manager && 'shop_subscription' === $post_object->name
						? \array_intersect_key( $args, self::get_connection_args( 'public' ) )
						: $args;
		...
		switch ( $access ) {
			case 'private':
				return array_merge(
					...
					array(
						'statuses'    => array(
							'type'        => array( 'list_of' => 'SubscriptionStatusEnum' ),
			...
			default:
				return array(
                    ...
                    'first' => array(
                        'type'        => 'Int',
                        'description' => __( 'Limit result for show orders per page.', 'wp-graphql-woocommerce' ),
                    ),
				);
		}
	}
	...
	public static function post_object_connection_query_args( $query_args, $source, $args, $context, $info ) {
		...
		$query_args['type'] = array('shop_subscription'); //\wc_get_order_types( 'view-orders' );
		if ( empty( $args['where'] ) || empty( $args['where']['statuses'] ) ) {
			$query_args['post_status'] = array_keys( \wcs_get_subscription_statuses() );
		}
	...
	public static function map_input_fields_to_wp_query( $query_args, $where_args, $source, $args, $context, $info, $post_type ) {
		...
		$prefixer = function( $status ) {
			$statuses = array_keys( \wcs_get_subscription_statuses() );
