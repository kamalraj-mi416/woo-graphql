		...
		if ( empty( $this->fields ) ) {
			...
			$fields = array(
				...
				'needsProcessing'       => function() {
                    $subscriptions_ids = \wcs_get_subscriptions_for_order( $this->wc_data->get_id(), array( 'order_type' => 'any' ) );
                    $proCatArr = [];
                    $order_item = end($this->wc_data->get_items());
                    ...
                    $proCats = get_the_terms ( $product_id, 'product_cat' );
                    foreach ( $proCats as $proCat ) {
                        $proCatArr[] = $proCat->slug;
                    }
                    if (in_array("betting", $proCatArr)) {
                        return false;
                    }

                    if(isset($subscriptions_ids) && !empty($subscriptions_ids)){
                        $sub = end( $subscriptions_ids);
                        if ( $this->wc_data->needs_payment() && $this->wc_data->get_id() == $sub->get_last_order( 'ids', 'any' ) ) {
                            return $this->wc_data->needs_payment();
                        }else{
                            return false;
                        }
                    }else{
                        return ! is_null( $this->wc_data->needs_payment() ) ? $this->wc_data->needs_payment() : false;
                    }
				},
				/**
