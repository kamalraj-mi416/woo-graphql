	...
	public static function mutate_and_get_payload() {
		return function( $input, AppContext $context, ResolveInfo $info ) {
			...
			if ( empty( $input['username'] ) ) {
                throw new UserError( __( 'Please enter username.', 'wp-graphql' ) );
            }

			if(!preg_match("/^[@a-zA-Z0-9-_]+$/", $input['username'])){
                throw new UserError( __( 'Please enter valid username.', 'wp-graphql' ) );
            }

            if ( empty( $input['email'] ) || !filter_var($input['email'], FILTER_VALIDATE_EMAIL) ) {
                throw new UserError( __( 'Please enter valid email address.', 'wp-graphql' ) );
            }
			...
			if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) && empty( $input['password'] ) ) {
				throw new UserError(
					__(  'Please provide a password.', 'wp-graphql-woocommerce' )
				);
			}
